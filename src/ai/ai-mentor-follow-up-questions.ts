...
const teachLessonTool = ai.defineTool(
    {
      ...
      description: "Explains a specific topic from a subject as a real teacher would. Use this when the user asks to be taught a lesson or topic (e.g., 'पाठ पढ़ाओ', ' समझाओ').",
      inputSchema: z.object({
          subject: z.string().describe("The subject of the lesson."),
          topic: z.string().describe("The topic of the lesson to be taught."),
      }),
      outputSchema: z.string(),
    },
    async ({ subject, topic }) => {
      // This tool doesn't need to return content, it just guides the LLM's response generation.
      // The actual teaching content will be generated by the prompt based on the subject and topic.
      return `Teaching lesson on topic: ${topic} in subject: ${subject}. The AI should now act as a teacher.`;
    }
);
